...
// Content item reference interface for clearer ID handling
export interface ContentItemReference {
  contentItemID: number;     // Reference ID (don't use for fetching)
  itemContainerID: number;   // Actual content ID (use this for get-content-item)
  contentViewID: number;     // Container/view ID
}

export interface ContentItemWithDetails extends ContentItemReference {
  content?: any;  // Full content object
  error?: string; // Error message if content fetch failed
}
 ...
 */
export function safeJsonParse(jsonString: string, context: string = 'Unknown'): any {
  try {
 ...
 */
export function validateApiResponse(response: any, expectedFields: string[], context: string): boolean {
 ...
 */
export function extractContainerIDs(contentListResponse: any): number[] {
  if (!contentListResponse?.items || !Array.isArray(contentListResponse.items) || contentListResponse.items.length === 0) {
    return [];
  }
  
  return contentListResponse.items[0].map((item: ContentItemReference) => item.itemContainerID);
}

/**
 * Calculate relevance score for search results
 */
function calculateRelevanceScore(fieldValue: string, searchTerm: string): number {
  ...
  if (field.endsWith(term)) return 70;
  
  // Contains search term as whole word
  const regex = new RegExp(`\\b${term}\\b`, 'i');
  if (regex.test(field)) return 60;
 ...
 */
export async function searchContentInContainer(
  apiClient: any,
  ...
  try {
    ...
    while (hasMoreItems) {
      ...
      const contentListResponse = await safeApiCall(
        () => apiClient.contentMethods.getContentItems(referenceName, guid, locale, currentParams),
        `fetch content list for search in ${referenceName}`,
        2,
        500
      ) as any;
      ...
      const batchPromises = batch.map(async (item: any) => {
        try {
          ...
          const fullContent = await safeApiCall(
            ...
            `fetch content item ${contentId} for search`,
            1, // Only 1 retry for search items
            300
          ) as any;
