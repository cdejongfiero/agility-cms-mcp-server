...
import { getRequiredEnvVar, getOptionalEnvVar, handleApiError, safeApiCall, validateApiResponse } from '../types/index.js';

// Load environment variables
dotenv.config();

interface GetContentItemInput {
  contentID: number;
  locale?: string;
}

class GetContentItemTool extends MCPTool<GetContentItemInput> {
  ...
  async execute(input: GetContentItemInput) {
    ...
    try {
      ...
      const contentItem = await safeApiCall(
        () => apiClient.contentMethods.getContentItem(input.contentID, guid, locale),
        `fetch content item ${input.contentID}`,
        3, // 3 retries
        1000 // 1 second delay
      );
