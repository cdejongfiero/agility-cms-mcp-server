...
interface NamedContentModelInput {
  referenceName: string;
}

class NamedContentModelTool extends MCPTool<NamedContentModelInput> {
  ...
  async execute(input: NamedContentModelInput) {
    try {
      ...
      const contentModel = await apiClient.modelMethods.getModelByReferenceName(
        input.referenceName, 
        guid
      );
