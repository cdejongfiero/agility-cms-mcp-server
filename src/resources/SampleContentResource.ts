...
interface SampleContentData {
  totalSamples: number;
  samplesByModel: Record<string, ContentSample[]>;
  lastUpdated: string;
  modelCoverage: Array<{
    modelName: string;
    sampleCount: number;
...
class SampleContentResource extends MCPResource {
  ...
  private cachedSamples: SampleContentData | null = null;
  ...
  private async refreshCache(): Promise<void> {
    ...
    try {
      ...
      const samplesByModel: Record<string, ContentSample[]> = {};
      const modelCoverage: Array<{
        modelName: string;
        sampleCount: number;
        hasRichContent: boolean;
      }> = [];
      ...
      for (let i = 0; i < Math.min(allContainers.length, maxContainers); i++) {
        ...
        try {
          ...
          if (contentList?.items && contentList.items.length > 0) {
            ...
            contentList.items.forEach((item: any) => {
            ...
            const hasRichContent = contentList.items.some((item: any) => {
