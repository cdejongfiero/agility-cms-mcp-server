...
import { getRequiredEnvVar, safeApiCall } from '../types/index.js';
...
class GetContainerListTool extends MCPTool {
    ...
    async execute(input) {
        ...
        try {
            ...
            const containers = await safeApiCall(() => apiClient.containerMethods.getContainerList(guid), 'fetch container list', 3, // 3 retries
            1000 // 1 second delay
            );
            if (!containers) {
                return {
            ...
            if (includeDetails) {
                ...
                processedContainers = containers.map((container) => ({
                    ...container,
                    details: {
            ...
            const stats = {
                ...
                withContent: containers.filter((c) => c.contentDefinitionID).length,
                shared: containers.filter((c) => c.isShared).length,
                dynamic: containers.filter((c) => c.isDynamicPageList).length,
                categories: [...new Set(containers.map((c) => c.contentViewCategoryName || 'Uncategorized'))]
            };
