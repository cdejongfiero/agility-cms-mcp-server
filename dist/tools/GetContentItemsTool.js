...
import { getRequiredEnvVar, getOptionalEnvVar, createListParams, safeApiCall, validateApiResponse } from '../types/index.js';
// Load environment variables
dotenv.config();
class GetContentItemsTool extends MCPTool {
    ...
    async execute(input) {
        ...
        try {
            ...
            const contentList = await safeApiCall(() => apiClient.contentMethods.getContentItems(input.referenceName, guid, locale, listParams), `fetch content items for ${input.referenceName}`, 3, // 3 retries
            1000 // 1 second delay
            );
            if (!contentList) {
                return {
            ...
            if (validateResponses) {
                const isValid = validateApiResponse(contentList, ['totalCount'], `content list for ${input.referenceName}`);
                if (!isValid) {
                ...
                for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
                    ...
                    const batchPromises = batch.map(async (item, itemIndex) => {
                        try {
                            ...
                            const fullContent = await safeApiCall(() => apiClient.contentMethods.getContentItem(contentId, guid, locale), `fetch content item ${contentId}`, 2, // Fewer retries for individual items
                            500 // Shorter delay
                            );
                            if (!fullContent) {
                                return {
                            ...
                            if (validateResponses) {
                                validateApiResponse(fullContent, ['fields', 'properties'], `content item ${contentId}`);
                            }
                            return {
                                containerID: contentId,
                                contentItemID: item.contentItemID,
            ...
            const successCount = processedItems.filter((item) => !item.error).length;
            const errorCount = processedItems.filter((item) => item.error).length;
            return {
                success: true,
