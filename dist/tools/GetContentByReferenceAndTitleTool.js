...
class GetContentByReferenceAndTitleTool extends MCPTool {
    ...
    async execute(input) {
        try {
            ...
            const contentList = await safeApiCall(() => apiClient.contentMethods.getContentItems(input.referenceName, guid, locale, listParams), `fetch content items for title search in ${input.referenceName}`, 3, 1000);
            if (!contentList) {
                return {
                    success: false,
            ...
            for (const item of contentList.items[0]) {
                try {
                    const fullContent = await safeApiCall(() => apiClient.contentMethods.getContentItem(item.itemContainerID, guid, locale), `fetch content item ${item.itemContainerID} for title search`, 2, 500);
                    if (!fullContent) {
